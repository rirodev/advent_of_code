...
class Folder:
    ...
    def get_folders_of_or_above_size(self, threshold):
        dirs_below = []
        for name, item in self.items.items():
            if isinstance(item, Folder): 
                dirs_below += item.get_folders_of_or_above_size(threshold)
        if self.size() >= threshold:
            dirs_below.append(self)
        return dirs_below 
...
root_size = root.size()
avaliable_space = disk_space - root_size
space_to_free = needed_space - avaliable_space

dirs = root.get_folders_of_or_above_size(space_to_free)
for d in dirs:
 print(space_to_free, d.name, d.size())
